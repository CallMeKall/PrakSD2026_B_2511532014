package pekan1_2511532014;
import java.util.Scanner;
public class JamDriver2_2511532014 {
	public static void main(String[] args) {
		Scanner input = new Scanner(System.in);
		System.out.println("=== Program Driver Object Jam ===");
		//1. input Jam Pertama
		System.out.println("\n--- Input Jam 1 ---");
		Jam_2511532014 j1 = buatJamDariInput (input);
		//2. input Jam Kedua
		System.out.println("\n--- Input Jam 1 ---");
		Jam_2511532014 j2 = buatJamDariInput (input);
	
		//3.Tampilkan data
		System.out.println("\n--- Hasil Operasi ---");
		System.out.println("Jam 1 (String)		: " + j1.toString());
		System.out.println("Jam 2 (String)		: " + j2.toString());
		System.out.println("Jam 1 dalam detik	: " + j1.toSeconds());
		System.out.println("Jam 2 dalam detik	: " + j2.toSeconds());
		
		//4.Operasi Relasional (perbandigan)
		int perbandingan = j1.compareTo(j2);
		if (perbandingan > 0) {
			System.out.println("Status		: Jam 1 lebih lambat (setelah) jam 2");
		} else if (perbandingan < 0) {
			System.out.println("Status		: Jam 1 lebih awal (sebelum) jam 2");
		} else {
			System.out.println("Status		: Jam 1 dan jam 2 sama persis");
		}
		
		//5. Operasi Mastematika 
		System.out.println("Durasi (j1 ke j2)	: "+ Jam_2511532014.durasiDetik(j1, j2) + " detik"); 
		
		Jam_2511532014 jNext = j1.nextSecond();
		System.out.println("Jam 1 Detik Berikutnya: "+ jNext);
		
		Jam_2511532014 jPrev = j1.prevSecond();
		System.out.println("Jam 1 Detik Sebelumnya: "+ jPrev);
		
		//6. Operasi Penjumlahan Jam
		Jam_2511532014 jHasilPlus = j1.plus(j2);
		System.out.println("HAsil j1 + j2	: "+ jHasilPlus);
		
		input.close();
		System.out.println("\nProgram Selesai.");
	}
	
	private static Jam_2511532014 buatJamDariInput(Scanner sc) {
		int h, m, s;
		while (true) {
			System.out.println ("Masukan Jam (0-23)	: ");
			h= sc.nextInt();
			System.out.println ("Masukan Jam (0-59)	: ");
			m= sc.nextInt();
			System.out.println ("Masukan Jam (0-59)	: ");
			s= sc.nextInt();
			
			//memanggil method static invalid
			if (Jam_2511532014.isValid(h, m, s)) {
				return new Jam_2511532014(h,m,s);			
			} else {
				System.out.println("[Error] Input tidak valid Silahkan ulangi.\n");
			}
			System.out.println ("Masukan Jam (0-23)	: ");
		}
	}

}

